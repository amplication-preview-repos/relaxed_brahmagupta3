/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { ArgsType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { ErrorModelWhereInput } from "./ErrorModelWhereInput";
import { IsOptional, ValidateNested, IsInt } from "class-validator";
import { Type } from "class-transformer";
import { ErrorModelOrderByInput } from "./ErrorModelOrderByInput";

@ArgsType()
class ErrorModelFindManyArgs {
  @ApiProperty({
    required: false,
    type: () => ErrorModelWhereInput,
  })
  @IsOptional()
  @ValidateNested()
  @Field(() => ErrorModelWhereInput, { nullable: true })
  @Type(() => ErrorModelWhereInput)
  where?: ErrorModelWhereInput;

  @ApiProperty({
    required: false,
    type: [ErrorModelOrderByInput],
  })
  @IsOptional()
  @ValidateNested({ each: true })
  @Field(() => [ErrorModelOrderByInput], { nullable: true })
  @Type(() => ErrorModelOrderByInput)
  orderBy?: Array<ErrorModelOrderByInput>;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  skip?: number;

  @ApiProperty({
    required: false,
    type: Number,
  })
  @IsOptional()
  @IsInt()
  @Field(() => Number, { nullable: true })
  @Type(() => Number)
  take?: number;
}

export { ErrorModelFindManyArgs as ErrorModelFindManyArgs };
